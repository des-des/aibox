...
const getNextNodes = function(mat, current, next) {
  if (typeof current === 'undefined') {
    ...
    const newNodeIds = mat[current.getId()]
    const newNodes = createNodesFromLinks(current, newNodeIds)
    next = resolveConflicts(next.concat(newNodes))
    return mergeResults(
      current,
      newNodes,
...
const fillSpaces = function(nodes, links, createId) {
  return links.reduce((result, link) => {
    ...
    const childNode = getNodeById(nodes, link[1])
    const yDiff = childNode.getY() - parentNode.getY()
    ...
    if (yDiff === 1) {
      ...
      for (i = 0; i < yDiff - 1; i++) {
        const nextNode =
          createNodesFromLinks(parentNode, [createId()], 'stub')[0]
...
const resolveConflicts = function(nextNodes) {
  const sortedNodes = nextNodes.sort((lNode, rNode) => (
...
const compareX = function(x1, x2) {
  const x1Arr = x1.getX().split('')
  const x2Arr = x2.getX().split('')
  let i = 0
...
export const adjMat2Graph = adjacency => {
